        ...
        if(ae.getSource() == b1){
            ...
            try{
                ...
                if(user.equals("Admin")){
                    String q1="select * from login where user = '"+"Admin"+"'";
                    ResultSet rs = c.s.executeQuery(q1);
                    if(rs.next()){
                    str = "insert into login values('"+meter+"','"+s+"' ,'"+username+"', '"+name+"', '"+password+"', '"+user+"')";
                    c.s.executeUpdate(str); 
                    JOptionPane.showMessageDialog(null, "you new account request is sent to admins");
                    }
                    else{
                    str = "insert into login values('"+meter+"','"+s1+"' ,'"+username+"', '"+name+"', '"+password+"', '"+user+"')";
                    c.s.executeUpdate(str);
